...
# A store-load-use hazard. A store writes data to the memory, a following load reads this data,
...
_start: # prepare GPIO address (0x10004) and set registers for data sequence
	...
	sd x4, 24(x0)          # Store 5 to address 24 (4th double-word)
	ld x7, 24(x0)          # Data sequence: Load 5 from the memory to x7.
	add x10, x7, x5        # Data sequence: 5+6=x10=11 -> stall
	add x8, x6, x7         # Data sequence: x7 needs forwarding (5+7=x8=12)
	sub x3, x9, x7         # data sequence: 12-5=x3=7
	# Separate ...
	addi x0, x0, 0
	addi x0, x0, 0
	addi x0, x0, 0
